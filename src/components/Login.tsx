...
const Login: React.FC<propTypes> = ({open, onClose}) =>{
	return (
    <div
    ...
    `} onClick={onClose}>
      ...
      transition-all max-w-md ${open ? "scale-100 opacity-100" : "scale-110 opacity-0"}`} onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col gap-4">
          ...
          <button className=" w-full rounded-lg py-2 px-10 bg-sky-400 hover:bg-sky-600 text-white" onClick={onClose}>
            Login
          </button>
        </div>
      </div>
